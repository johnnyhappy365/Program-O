...
class AipNlp extends AipBase{
     ...
     * @var string
     */
    private $simnetUrl = 'https://aip.baidubce.com/rpc/2.0/nlp/v1/simnet';
    ...
    /**
     * 短文本相似度
     * @param  string $query1 自然语言1
     * @param  string $query1 自然语言2
     * @param  array $options 可选参数
     * @return array
     */
    public function simnet($query1, $query2, $options=array()){

        $data = array();
        $data['input'] = array(
            'qslots' => array(
                array(
                    'terms_sequence' => urlencode(mb_convert_encoding($query1, 'GBK', 'UTF8')),
                    'type' => 0,
                    'items' => array(),
                ),
            ),
            'tslots' => array(
                array(
                    'terms_sequence' => urlencode(mb_convert_encoding($query2, 'GBK', 'UTF8')),
                    'type' => 0,
                    'items' => array(),
                ),
            ),
